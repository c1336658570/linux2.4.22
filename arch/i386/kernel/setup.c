...
// 该函数在内核启动时进行体系结构相关的设置
// 此函数作用是进行系统启动时的各种初始化工作，包括设置内存区域，解析命令行参数，初始化分页，扫描机器的DMI信息，解析ACPI表等
void __init setup_arch(char **cmdline_p)
{
	unsigned long max_low_pfn;

#ifdef CONFIG_VISWS
	// 如果定义了CONFIG_VISWS，执行与VISWS（Visual Workstation）相关的设置。
	visws_get_board_type_and_rev();
#endif

#ifndef CONFIG_HIGHIO	// 如果没有定义CONFIG_HIGHIO，设置blk_nohighio为1。
	blk_nohighio = 1;
#endif
 	ROOT_DEV = to_kdev_t(ORIG_ROOT_DEV);		// 设置根设备。
 	drive_info = DRIVE_INFO;				// 设置驱动器信息。
 	screen_info = SCREEN_INFO;			// 设置屏幕信息。
	apm_info.bios = APM_BIOS_INFO;	// 设置APM（Advanced Power Management）信息。
	if( SYS_DESC_TABLE.length != 0 ) {	// 如果系统描述表的长度不为0，则设置相关的系统信息。
		MCA_bus = SYS_DESC_TABLE.table[3] &0x2;
		machine_id = SYS_DESC_TABLE.table[0];
		machine_submodel_id = SYS_DESC_TABLE.table[1];
		BIOS_revision = SYS_DESC_TABLE.table[2];
	}
	aux_device_present = AUX_DEVICE_INFO;	// 设置辅助设备的存在信息。

#ifdef CONFIG_BLK_DEV_RAM	// 如果定义了CONFIG_BLK_DEV_RAM，设置RAM磁盘的相关参数。
	...
	if (!MOUNT_ROOT_RDONLY)		// 如果根文件系统不是只读的，修改 root_mountflags。
		root_mountflags &= ~MS_RDONLY;
	// 初始化内存管理的起始代码、结束代码、结束数据和断点地址。
	...
	// 在这一点之前，不允许使用bootmem分配器分配任何内存。
	/*
	 * NOTE: before this point _nobody_ is allowed to allocate
	 * any memory using the bootmem allocator.
	 */

#ifdef CONFIG_SMP		// 如果定义了CONFIG_SMP，为AP处理器在低内存中分配实模式栈。
	smp_alloc_memory(); /* AP processor realmode stacks in low memory*/
#endif
	paging_init();		// 初始化分页。

	dmi_scan_machine();		// 扫描DMI（Desktop Management Interface）信息。

	/*
	 * Parse the ACPI tables for possible boot-time SMP configuration.
	 */
	acpi_boot_init();	// 解析ACPI（Advanced Configuration and Power Interface）表，以获取可能的引导时SMP（Symmetric Multi-Processing）配置。

#ifdef CONFIG_X86_LOCAL_APIC
	/*
	 * get boot-time SMP configuration:
	 */
	if (smp_found_config)	// 如果找到了SMP配置，获取SMP配置。
		get_smp_config();
#endif

	register_memory(max_low_pfn);	// 注册内存。

#ifdef CONFIG_VT
#if defined(CONFIG_VGA_CONSOLE)	// 如果定义了CONFIG_VT，设置控制台切换指针。
	conswitchp = &vga_con;
